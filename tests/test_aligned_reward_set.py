...
@given(
    reward=arrays(dtype=np.float32, shape=(5,), elements=floats(-1.0, 1.0, width=32)).filter(
        lambda r: np.any(r != 0)
    ),
    n_states=integers(min_value=2, max_value=100),
...
def test_aligned_reward_set_consistent(
    ...
    diffs = make_aligned_reward_set(reward, n_states, n_trajs, env, policy, tqdm=False)
